...
struct FlatRegionResult {
  TiledHloRegion region;
  std::vector<const TiledHloInstruction*> canonical_roots;
};

static absl::StatusOr<FlatRegionResult> CreateFlatRegion(
  ...
  while (!worklist.empty()) {
    ...
    for (const auto& region_root_ids : spec.region_roots) {
      ...
      ASSIGN_OR_RETURN(FlatRegionResult res,
  ...
  return FlatRegionResult{std::move(tiled_hlo_instructions),
...
/*static*/ absl::StatusOr<TiledHloRegion> TiledHloComputation::CreateHloRegion(
  ...
  ASSIGN_OR_RETURN(FlatRegionResult res,
                   CreateFlatRegion(std::move(roots), fusion, tiling_space,
                                    rt_symbol_to_tiled_hlo));
