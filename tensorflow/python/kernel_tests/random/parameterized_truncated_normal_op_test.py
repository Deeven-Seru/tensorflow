...
class ParameterizedTruncatedNormalTest(test.TestCase):
  ...
  def testIntegerOverflow(self):
    ...
    with self.session(use_gpu=False):
      ...
      # Size equal to INT32_MAX should pass the limit check but fail validation.
      # Wrap in a CPU device block so parameter validation is actively executed.
      with ops.device("/cpu:0"):
        with self.assertRaisesRegex(
            (ValueError, errors.InvalidArgumentError), "Invalid parameters"
        ):
          samples_max = random_ops.parameterized_truncated_normal(
              shape=[2147483647],
              means=0.0,
              stddevs=1.0,
              minvals=[3.0],
